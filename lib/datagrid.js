class DataGrid {
    ...
    handleDrop(event) {
        ...
        if (targetRow && targetRow !== this.draggedRow) {
            ...
            if (draggedRowIndex !== -1 && targetRowIndex !== -1) {
                ...
                // Update the filtered data to reflect the new order
                this.filterData(false); // Pass false to avoid resetting the page number
    ...
    populateDataGrid(data) {
        ...
        data.forEach((item, index) => {
            ...
            row.classList.toggle('selected-row', this.selectedRows.has(item.id));
            if (index % 2 === 0) {
